...
class CVEAnalyzerPanel(JPanel):
    ...
    def _build_ui(self):
        ...
        col_model.getColumn(3).setPreferredWidth(85)   # Status (verification)
        col_model.getColumn(4).setPreferredWidth(300)  # Description
        col_model.getColumn(5).setPreferredWidth(100)  # Affected
    ...
    def _on_cve_selected(self, event):
        ...
        if entry:
            ...
            vs_color = Color(0, 128, 0) if vs == "VERIFIED" else Color(204, 0, 0)
            append_to_pane(self.poc_pane, "# Status: ")
            append_to_pane(self.poc_pane, "%s\n" % vs, bold=True, color=vs_color)
            append_to_pane(self.poc_pane, "# %s\n\n" % entry.description)
        ...
        try:
            ...
            for cve in data.get("cves", []):
                entries.append(CVEEntry(
                    ...
                    verification_status=cve.get("verification_status", "UNVERIFIED"),
                ))
        except Exception:
            pass
        return entries
