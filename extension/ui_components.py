...
# Maximum characters kept in a JTextPane before old content is trimmed.
# JTextPane + StyledDocument slows dramatically beyond ~100k chars.
_PANE_MAX_CHARS = 80000
_PANE_TRIM_TO = 60000  # after trimming, keep this many chars (from the end)


def append_to_pane(text_pane, text, color=None, bold=False, size=12):
    """Append styled text to a JTextPane, trimming old content if needed."""
    doc = text_pane.getStyledDocument()

    # Trim old content if document is too large
    current_len = doc.getLength()
    if current_len > _PANE_MAX_CHARS:
        remove_count = current_len - _PANE_TRIM_TO
        try:
            doc.remove(0, remove_count)
        except Exception:
            pass
